import { ChatInputCommandInteraction, EmbedBuilder, SlashCommandBuilder, Team, User } from 'discord.js';
...
function createBotInfoEmbed(botUser: User, interaction: ChatInputCommandInteraction, owners: string): EmbedBuilder {
    ...
    return new EmbedBuilder()
        ...
        .setAuthor({ iconURL: botUser.displayAvatarURL(), name: botUser.username })
        .setThumbnail(botUser.displayAvatarURL())
        .setURL('https://apprendre-discord.fr')
        .setDescription("I was created to help as many people as possible on discord.")
        .addFields(
            { inline: true, name: 'Creation date:', value: `<t:${Math.floor(botUser.createdTimestamp / oneSecondInMs)}:R>` },
            { inline: true, name: 'Online since', value: `<t:${Math.floor(interaction.client.readyTimestamp / oneSecondInMs)}:f>` },
            { inline: true, name: 'Owner(s):', value: owners },
            { inline: true, name: 'My developers:', value: `▸ [Guscraftin](https://github.com/Guscraftin)` },
        )
        .setColor('DarkAqua')
        .setTimestamp()
